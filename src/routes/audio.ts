  ...
  try {
      ...
      try {
        ...
        try {
          ...
          try {
            ...
            console.warn('[finalize] DeepSeek raw HTTP fallback failed', e2 && (e2.stack || e2));
            const lines = fullText.split('\n').map(s => s.trim()).filter(Boolean);
    ...
    if (email && summary && process.env.RESEND_API_KEY) {
      try {
        ...
        try {
          // also append a short trace line to tmp/mail.log so disk records include finalize attempts
          const logLine = `[${new Date().toISOString()}] FINALIZE_ATTEMPT to=${email} room=${room} source=finalize` + '\n';
          await fs.promises.mkdir(path.join(__dirname, '..', '..', 'tmp'), { recursive: true }).catch(() => {});
          await fs.promises.appendFile(path.join(__dirname, '..', '..', 'tmp', 'mail.log'), logLine);
        } catch (e) {
          console.warn('[finalize] failed appending finalize attempt to tmp/mail.log', e);
        }
      } catch (e) {
        console.warn('[finalize] sendSummaryByEmail failed for explicit email', { to: email, err: e && (e.stack || e) });
      }
      ...
      try {
        ...
        if (backend) {
            ...
            if (participants && participants.length) {
              for (const addr of participants) {
                try {
                  ...
                  try {
                    const logLine = `[${new Date().toISOString()}] FINALIZE_SENT to=${addr} room=${room} source=finalize` + '\n';
                    await fs.promises.mkdir(path.join(__dirname, '..', '..', 'tmp'), { recursive: true }).catch(() => {});
                    await fs.promises.appendFile(path.join(__dirname, '..', '..', 'tmp', 'mail.log'), logLine);
                  } catch (e) {
                    console.warn('[finalize] failed appending finalize sent to tmp/mail.log', e);
                  }
                } catch (e) {
                  console.warn('[finalize] failed emailing to participant', addr, e && (e.stack || e));
                  try {
                    const logLine = `[${new Date().toISOString()}] FINALIZE_FAILED to=${addr} room=${room} err=${(e && (e.message || e.toString())) || 'unknown'}` + '\n';
                    await fs.promises.mkdir(path.join(__dirname, '..', '..', 'tmp'), { recursive: true }).catch(() => {});
                    await fs.promises.appendFile(path.join(__dirname, '..', '..', 'tmp', 'mail.log'), logLine);
                  } catch (e2) {
                    console.warn('[finalize] failed appending finalize failure to tmp/mail.log', e2);
                  }
                }
              }
            } else {
          ...
          try {
            ...
            if (unique.length) {
              ...
              for (const addr of unique) {
                try {
                  ...
                  try {
                    const logLine = `[${new Date().toISOString()}] FINALIZE_SENT_EXTRACT to=${addr} room=${room} source=finalize_fallback` + '\n';
                    await fs.promises.mkdir(path.join(__dirname, '..', '..', 'tmp'), { recursive: true }).catch(() => {});
                    await fs.promises.appendFile(path.join(__dirname, '..', '..', 'tmp', 'mail.log'), logLine);
                  } catch (e) { console.warn('[finalize] failed appending finalize sent fallback to tmp/mail.log', e); }
                } catch (e) {
                  console.warn('[finalize] failed emailing to extracted participant', addr, e && (e.stack || e));
                  try {
                    const logLine = `[${new Date().toISOString()}] FINALIZE_FAILED_EXTRACT to=${addr} room=${room} err=${(e && (e.message || e.toString())) || 'unknown'}` + '\n';
                    await fs.promises.mkdir(path.join(__dirname, '..', '..', 'tmp'), { recursive: true }).catch(() => {});
                    await fs.promises.appendFile(path.join(__dirname, '..', '..', 'tmp', 'mail.log'), logLine);
                  } catch (e2) { console.warn('[finalize] failed appending finalize failure fallback to tmp/mail.log', e2); }
                }
              }
            } else {
            ...
            console.warn('[finalize] email-extraction fallback failed', e && (e.stack || e));
          }
        }
      } catch (e) {
    ...
    // Cleanup transcript files after finalizing (only the ones we aggregated)
    try {
      ...
      try {
        ...
        for (const f of matched) {
          try {
            ...
            filesData.push({ name: f, preview: `Failed to read: ${String(e?.message || e)}` });
          }
        }
